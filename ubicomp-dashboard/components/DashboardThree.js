...
export default function DashboardThree() {
  ...
  // ... existing useEffect and helper functions (isLikelyPhone, ProfileTag) ...
  // Ensure the useEffect from the previous response is used here.
                    ...
                    <li key={profile.id || profile.profile_name} className={itemClasses}>
                      ...
                      <div className="ml-1 text-sm text-gray-700 space-y-1">
                        ...
                        {profile.profile_type === 'absence' && profile.provocative_note_final &&
                           <p className="mt-2 font-semibold text-red-700">⚠️ {profile.provocative_note_final}</p> // Added warning emoji for absence
                        }
                      </div>
                    </li>
                  );
...
// ... Make sure isLikelyPhone and ProfileTag functions are defined as in the previous response ...
// The useEffect hook should also be the one from the previous comprehensive update.
// For brevity, I'm only showing the return part of DashboardThree and the SQL.
// You need to ensure the full component structure from the previous answer is maintained,
// especially the useEffect hook for fetching and processing data.
...
const ProfileTag = ({ profileType, isHighConcern }) => {
  ...
  if (isHighConcern) {
    ...
    text = `⚠️ ${text}`; // Using emoji here for consistency
  } else if (profileType === 'absence') {
    bgColor = 'bg-red-400';
    textColor = 'text-red-900';
    // text = `🚫 ${text}`; // Optional: emoji for absence tag
